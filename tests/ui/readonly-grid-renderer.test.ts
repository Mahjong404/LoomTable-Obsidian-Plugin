...
describe("ReadonlyGridRenderer", () => {
  it("renders only the fixed-height viewport window for a large result page", () => {
    ...
    expect(container.querySelectorAll(".loom-grid-editable")).not.toHaveLength(
      0,
    );
  ...
  it("commits Tab and Shift+Tab edits while keeping focus in the adjacent cell", () => {
    ...
    firstCell?.click();
    const firstEditor =
      container.querySelector<HTMLInputElement>(".loom-grid-editor");
  ...
  it("confirms Conflict Overwrite before invoking the recovery callback", async () => {
    ...
    document
      ...
      ?.click();
    await vi.waitFor(() =>
      expect(callbacks.onConflictAction).toHaveBeenCalledWith(
        "record_01",
        "overwrite",
      ),
    );
  ...
  it("keeps raw transport details behind the Grid diagnostic disclosure", () => {
    ...
    expect(
      container.querySelector(".loom-grid-status .loom-diagnostic summary")
        ?.textContent,
    ).toBe("Error details");
    expect(
      container.querySelector(".loom-grid-status .loom-diagnostic pre")
        ?.textContent,
    ).toContain("req_01");
  });
  ...
  it("keeps a visible status when the data source is offline", () => {
    ...
    expect(container.querySelector(".loom-grid-status")?.textContent).toContain(
      "offline",
    );
  });
  ...
  it("distinguishes Location values that are unset, unlocated, and not renderable", () => {
    ...
    expect(
      container.querySelector('[data-field-id="field_location"]')?.textContent,
    ).toBe("Unset");
  ...
  it("keeps offline Grid Cells read-only and does not start an editor", () => {
    ...
    renderer.render(
      createState(1, { status: "offline", error: { message: "offline" } }),
    );
  ...
  it("shows Server and local values with explicit conflict actions", () => {
    ...
    expect(
      container.querySelector(".loom-grid-conflict-values")?.textContent,
    ).toContain("Server value");
    expect(
      container.querySelector(".loom-grid-conflict-values")?.textContent,
    ).toContain("mutation_01");
    expect(
      container.querySelector(".loom-grid-conflict-values")?.textContent,
    ).toContain("failedCommandIndex");
    expect(
      container.querySelector(".loom-grid-conflict-intent")?.textContent,
    ).toContain("Local value");
    expect(
      container.querySelector(".loom-grid-conflict-intent")?.textContent,
    ).toContain("field_archived");
    expect(
      container.querySelector(".loom-grid-conflicts")?.getAttribute("role"),
    ).toBe("region");
    expect(
      container
        .querySelector(".loom-grid-conflicts")
    ...
    const buttons = container.querySelectorAll<HTMLButtonElement>(
      ".loom-grid-conflict button",
    );
    buttons[0]?.click();
    buttons[1]?.click();
    expect(callbacks.onConflictAction).toHaveBeenNthCalledWith(
      1,
      "record_01",
      "use-server",
    );
  ...
  it("renders idempotency key reuse as a terminal safety error without retry", () => {
    ...
    expect(
      container.querySelector(".loom-grid-edit-status")?.textContent,
    ).toContain("already associated with a different request");
    expect(container.querySelector(".loom-grid-edit-status button")).toBeNull();
    expect(
      container.querySelector(".loom-save-status")?.textContent,
    ).not.toContain("Saved");
  });
  ...
  it("offers an explicit retry action for a failed save", () => {
    ...
    container
      .querySelector<HTMLButtonElement>(".loom-grid-edit-status button")
      ?.click();
...
function createState(
  ...
  const records: readonly LoomTableRecord[] = Array.from(
    { length: recordCount },
    (_, index) => ({
