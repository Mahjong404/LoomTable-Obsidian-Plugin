...
describe("Record Detail attachment lifecycle actions", () => {
  it("confirms Detach with the attachment name and leaves data untouched on Cancel", async () => {
    ...
    const action = detail.querySelector<HTMLButtonElement>(
      ".loom-attachment-detach-action",
    );
    expect(action).not.toBeNull();
    action?.click();
    await vi.waitFor(() =>
      expect(confirmDangerousAction).toHaveBeenCalledTimes(1),
    );
  ...
  it("passes the selected reference to Detach only after explicit confirmation", async () => {
    ...
    detail
      .querySelector<HTMLButtonElement>(".loom-attachment-detach-action")
      ?.click();
  ...
  it("shows a Retry action only after a retryable upload failure", async () => {
    ...
    const add = createAttachmentAddCallback(
      {
        initializeAttachment: vi
          .fn()
          .mockRejectedValue(
            new LoomTableClientError("network", { message: "temporary" }),
          ),
    ...
    detail
      .querySelector<HTMLButtonElement>(".loom-attachment-add-action")
      ?.click();
    await vi.waitFor(() =>
      expect(
        detail.querySelector(".loom-attachment-add-retry-action"),
      ).not.toBeNull(),
    );
    expect(
      detail.querySelector(".loom-attachment-add-retry-action")?.textContent,
    ).toContain("Retry");
  });
});
