...
describe('MutationQueueScheduler', () => {
  it('recovers a persisted sending entry as queued before any scheduling', async () => {
    ...
    await store.replace({ schemaVersion: 1, entries: [entry({ state: 'sending' })] });
  ...
  it('keeps one Record FIFO while allowing different Records to run in parallel', async () => {
        ...
        async (_tableId: string, request: MutationRequest): Promise<MutationResult> => {
          ...
          if (command.recordId === 'record_01' && command.set?.field_a === 'one') {
            return new Promise((resolve) => {
              releases.set('record_01-one', () => resolve(result(request.clientMutationId, 'record_01', 2)));
            });
          }
          if (command.recordId === 'record_02') {
            return new Promise((resolve) => {
              releases.set('record_02-two', () => resolve(result(request.clientMutationId, 'record_02', 2)));
            });
          }
    ...
    expect(calls[0]?.commands[0]).toMatchObject({ expectedRevision: 1, set: { field_a: 'one' } });
    expect(calls[2]?.commands[0]).toMatchObject({ expectedRevision: 1, set: { field_a: 'two' } });
    scheduler.stop();
  });
