...
import type { MapRenderer, MapRendererEventListener } from '../../src/maps/renderer/map-renderer';
...
describe('MapViewController A1 lifecycle contract', () => {
  it('rejects a deleted or foreign-table Location Field without querying Map data', async () => {
    ...
    const controller = createController(createClient({ summarizeMap }), createMapView(), [
  ...
  it('stores the Map change cursor and falls back to Record ID for empty Point text', async () => {
    ...
    expect(renderer.features[0]).toMatchObject({ primaryFieldText: 'record_untitled' });
  });
