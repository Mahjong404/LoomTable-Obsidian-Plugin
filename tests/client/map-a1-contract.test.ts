...
describe('HttpLoomTableClient Map A1 contract', () => {
  it('sends Summary as a bodyless POST and preserves invalid-coordinate counts', async () => {
    ...
    await expect(createClient(transport).summarizeMap('view/07')).resolves.toMatchObject({
      summary: {
        matchedRecordCount: 3,
        renderableRecordCount: 1,
        unlocatedRecordCount: 1,
  ...
  ] as const)('maps HTTP %d %s to refreshable cursor expiry', async (status, code) => {
    const transport = queuedTransport([
      jsonResponse(status, {
        error: { code, message: 'Refresh the Map query.', requestId: `req_${status}`, details: {} },
      }),
    ]);
