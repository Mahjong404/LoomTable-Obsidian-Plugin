...
export function createRecordDetail(
  ...
  const heading = createText(
    "h2",
    options.translate("record.details") + ": " + record.id,
  );
  ...
  const fields =
    options.fields.length > 0 ? options.fields : fallbackFields(record);
...
function renderField(
  ...
  if (field.type === "location") {
    ...
    const onAttachmentDownload =
      ...
      options.callbacks?.onAttachmentDownload !== undefined
        ? (attachment: RenderedAttachment): void | Promise<void> => {
            if (attachment.id === undefined) return;
            return options.callbacks?.onAttachmentDownload?.(
              record.id,
              field.id,
              attachment,
            );
    ...
    const onAttachmentOpen =
      field.type === "attachment" &&
      options.callbacks?.onAttachmentOpen !== undefined
        ? (attachment: RenderedAttachment): void | Promise<void> => {
            if (attachment.id === undefined) return;
            return options.callbacks?.onAttachmentOpen?.(
              record.id,
              field.id,
              attachment,
            );
    ...
    const onAttachmentPreview =
      field.type === "attachment" &&
      options.callbacks?.onAttachmentPreview !== undefined
        ? (attachment: RenderedAttachment): void | Promise<void> => {
            if (attachment.id === undefined) return;
            return options.callbacks?.onAttachmentPreview?.(
              record.id,
              field.id,
              attachment,
            );
    ...
    const onAttachmentDetach =
      field.type === "attachment" &&
      options.callbacks?.onAttachmentDetach !== undefined
...
function renderLocationValue(
  ...
  if (raw === undefined) {
    ...
    wrapper.append(
      createLocationStatus("unset", options.translate("record.field.unset")),
    );
  } else if (raw === null) {
    ...
    wrapper.append(
      createLocationStatus(
        "cleared",
        options.translate("record.field.cleared"),
      ),
    );
    ...
    for (const [key, messageKey] of [
      ...
      details.append(
        createText("dt", options.translate(messageKey)),
        createText("dd", text),
      );
    }
    wrapper.append(
      details,
      createLocationStatus(state, locationStatusText(state, options)),
    );
    if (coordinates !== null) {
      ...
      if (
        ...
        open.addEventListener(
          "click",
          () =>
            void options.callbacks?.onOpenLocationInMap?.(
              record.id,
              field.id,
              location,
            ),
      ...
      copy.setAttribute(
        "aria-label",
        options.translate("record.location.copy"),
      );
      ...
      wrapper.append(
        copy,
        createPreviewTrigger(coordinates, options.translate),
      );
    }
  } else {
  ...
  edit.disabled =
    options.offline === true || options.callbacks?.onLocationEdit === undefined;
...
async function copyCoordinates(
  ...
  try {
    if (options.callbacks?.onCopyCoordinates !== undefined) {
      await options.callbacks.onCopyCoordinates(
        record.id,
        field.id,
        coordinates,
      );
...
function createLocationEditor(
  ...
  precision.setAttribute(
    "aria-label",
    options.translate("record.location.precision"),
  );
  const emptyPrecision = document.createElement("option");
  emptyPrecision.value = "";
  emptyPrecision.textContent = options.translate(
    "record.location.precision.none",
  );
  ...
  const submit = async (intent: LocationEditIntent): Promise<void> => {
    ...
    try {
      ...
      const nextRecord =
        updatedRecord instanceof Object ? updatedRecord : record;
      const nextValue =
        updatedRecord instanceof Object
          ...
          : intent.kind === "unset"
            ...
            : intent.kind === "clear"
              ? null
              : intent.value;
      const next = renderLocationValue(
        nextRecord,
        field,
        nextValue,
        options,
      ...
      if (conflict !== undefined) {
        ...
        const conflictBox = renderConflict(
          record.id,
          conflict,
          options,
          detailRoot,
  ...
  const confirmAndSubmit = (
    ...
    if (saving) return;
    trigger.focus();
    void requestDangerousConfirmation(options, root, message, trigger).then(
      (confirmed) => {
...
function renderConflict(
  ...
  serverValues.setAttribute(
    "aria-label",
    options.translate("record.serverValue"),
  );
  ...
  localIntent.setAttribute(
    "aria-label",
    options.translate("record.localIntent"),
  );
  ...
  discardAll.addEventListener("click", () => {
    if (!confirmDiscard(options, options.translate("record.discardAllConfirm")))
      return;
    invoke("discard-all");
  });
...
function confirmDiscard(
  ...
  if (options.confirmDiscard !== undefined)
    return options.confirmDiscard(message);
...
function locationPresentationState(
  ...
  return Math.abs(coordinates.lat) > MAX_RENDERABLE_LATITUDE
    ? "unrenderable"
    : "located";
}
...
function locationStatusText(
  ...
  if (state === "unrenderable")
    return options.translate("record.location.unrenderable");
...
function formatPrecision(value: string, translate: Translator): string {
  ...
  if (value === "rooftop")
    return translate("record.location.precision.rooftop");
  if (value === "approximate")
    return translate("record.location.precision.approximate");
...
function createAttachmentAddAction(
  ...
  const setIdle = (): void => {
    ...
    if (retryAction !== null) {
      ...
      retryAction.textContent = options.translate(
        "record.attachment.action.retry",
      );
  ...
  const ensureRetryAction = (): void => {
    if (
      retryAction !== null ||
      options.callbacks?.onAttachmentAddRetry === undefined
    )
      return;
  ...
  const run = async (retry: boolean): Promise<void> => {
    ...
    try {
      ...
      if (updated === null) {
        status.textContent = options.translate(
          "record.attachment.action.addCancelled",
        );
        return;
      }
      if (updated !== undefined) onRecordUpdated(updated);
      ...
      status.textContent = describeAttachmentUploadError(
        error,
        options.translate,
      );
