...
description: Guidance for distinctive, intentional visual design when building new UI or reshaping an existing one. Helps with aesthetic direction, typography, and making choices that don't read as templated defaults. In the LoomTable repo this skill is scoped to visual refinement only — see the local adaptation note below.
license: Complete terms in LICENSE.txt
---

# Frontend Design

> **Local adaptation (LoomTable).** Upstream: `anthropics/skills` → `skills/frontend-design` @ `34040c9`. In this repository, this skill is a **visual-refinement specialist**, not an art director:
>
> - The visual system already exists and is canonical: `docs/ui/design-system.md` + the `--loom-*` tokens in `styles.css`. Refine within it — never invent a new visual identity, palette, type system, or icon language for LoomTable.
> - "Distinctive" here means better hierarchy, spacing, density, alignment, and feedback — not differentiation from other products. The product deliberately blends into the Obsidian host.
> - The brief is always LoomTable's own docs. Where upstream text below says "the client's brief," read it as `loomtable-ui-design` + `docs/ui/` specifications, which always win over the generic guidance in this file.
> - Do not propose new runtime dependencies, frameworks, fonts, or color tokens. The anti-default list below is useful calibration; apply its *restraint* sections in full, and its *distinctiveness* sections only within the LoomTable token system.
>
> Everything below this note is the upstream skill verbatim.
