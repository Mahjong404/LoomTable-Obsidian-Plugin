# Grid 视觉风格对比：外部表格组件库 / 外部多维表产品 / LoomTable

数据来源：外部表格组件库 已安装版本的 `styles.css` 与 README（可逐行核对）；外部多维表产品 仓库结构（`packages/sdk/src/components/grid`，canvas 渲染架构）与其产品截图；LoomTable 当前 `styles.css`。

**重要架构差异**：外部多维表产品 的表格是 canvas 逐层渲染（RenderLayer / InteractionLayer / TouchLayer / InfiniteScroller），视觉上是 外部多维表产品 式"画布网格"；外部表格组件库 与 LoomTable 都是 DOM 网格。因此 外部表格组件库 是更直接的参照；外部多维表产品 主要提供交互模式参照（面板、视图栏、记录展开），视觉细节以 外部表格组件库 为准。

## 一、网格线与边框（"无边框"观感的真实来源）

| 项 | 外部表格组件库 | 外部多维表产品 | LoomTable 现状 |
|---|---|---|---|
...
## 七、外部多维表产品 独有能力（LoomTable 未覆盖，供参考不照搬）

- canvas 网格 → 任意行列数流畅滚动、单元格内富渲染（进度条、评分星、成员头像）
- 拖拽列宽/列序、行拖拽排序、范围选择 + 填充柄批量填充
- 看板/画廊/日历等多视图类型；字段类型更全（公式、汇总、关联、成员）
- 右键上下文菜单（截图 1 中 外部表格组件库 也有：编辑/复制/填充/批量修改/单元格样式/打开详情/绑定笔记/插入行/删除行列）
...
### D1. 视图页签形态
- **A（外部表格组件库 式）**：圆角药丸，激活 = accent 12% 底 + accent 字重 600。**已应用此方案。**
- B（外部多维表产品 式）：下划线指示条页签——更"文档感"但 Obsidian 内不常见。
- C：维持现状有框按钮。
...
- **A**：加（外部表格组件库/外部多维表产品 都有，扫描性更好，需要一套 14×14 图标映射 ~15 种字段类型）
- B：维持纯文本（省事，列头更矮更干净）

### D3. 行内动作（打开详情/删除）显现时机

- **A（外部表格组件库 式）**：行 hover / focus-within 时才显示 ↗ 和 ×，平时只显示行号。**已应用。**
- B：常驻（现状改进前）。

### D4. 单元格选中/活动态
- **A（外部表格组件库 式）**：活动单元格 = accent 10% 底色 + 2px 内边框，列头选中整列高亮——需要新增 selection 状态模型
...
### D5. "新增记录"按钮层级
- **A（外部表格组件库 式）**：accent 实底主按钮（紫色"新增行"位）——**已应用。**
- B：普通工具按钮。

### D6. 右键上下文菜单

- 外部表格组件库 有完整右键菜单（编辑/复制/清空/填充/样式/详情/绑定笔记/插行/删行列）；外部多维表产品 也有。
...
### D7. 命令栏分隔线与按钮密度
- **A（外部表格组件库 式）**：工具组间 1px 竖分隔线，按钮 30px 高 gap 1–2px 紧凑排列——**已应用。**
- B：维持当前间距。

### D8. Detail 面板模态变体

- **A**：仅右侧浮层（现状，贴 外部表格组件库 默认）
